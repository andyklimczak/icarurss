defmodule IcarurssWeb.UserLive.Registration do
  ...
  def mount(_params, _session, %{assigns: %{current_scope: %{user: user}}} = socket)
    ...
    {:ok, redirect(socket, to: IcarurssWeb.UserAuth.signed_in_path(socket))}
  end
  ...
  def mount(_params, _session, socket) do
    if Accounts.registration_enabled?() do
      ...
      {:ok,
       socket
       |> put_flash(:error, "Registration is disabled. Ask an admin for an invite.")
    ...
    if Accounts.registration_enabled?() do
      case Accounts.register_user_with_password(user_params) do
        {:ok, user} ->
          {:noreply,
           socket
           |> put_flash(:info, "Account created for #{user.username}. You can now log in.")
           |> push_navigate(to: ~p"/users/log-in")}
